...
hexo.extend.filter.register('after_post_render', data => {
    ...
    if (theme.lazyload) {
        data.content = data.content.replace(/(<img[^>]*) src=/img, '$1 src=');
    }
    if (theme.exturl) {
        ...
        data.content = data.content.replace(/<a[^>]*\shref="([^"]+)"[^>]*>([^<]+)<\/a>/ig, (match, href, html) => {
          ...
          if (title) return `<span class="exturl" data-url="${encoded}" title="${title[1]}">${html}</span>`;
    
          return `<span class="exturl" data-url="${encoded}">${html}</span>`;
        });
      }

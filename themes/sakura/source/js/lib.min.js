...
//Player.js
var NOWPLAYING=null;const isMobile=/mobile/i.test(window.navigator.userAgent),store={get:function(item){return localStorage.getItem(item)},set:function(item,str){return localStorage.setItem(item,str),str},del:function(item){localStorage.removeItem(item)}},showtip=function(msg){if(msg){var tipbox=document.body.appendChild(document.createElement("div"));tipbox.innerHTML=msg,tipbox.className="tip",setTimeout((function(){tipbox.classList.add("hide"),setTimeout((function(){document.body.removeChild(tipbox)}),300)}),3e3)}},tabFormat=function(){var tabs=document.querySelectorAll("div.tab"),tabGroups={};tabs.forEach((function(element,index){if(!element.getAttribute("data-ready")){var id=element.getAttribute("data-id"),title=element.getAttribute("data-title");tabGroups[id]||(tabGroups[id]=[]),tabGroups[id].push({element:element,title:title})}})),Object.keys(tabGroups).forEach((function(id){var group=tabGroups[id];if(group.length<=1)group.forEach((function(item){item.element.setAttribute("data-ready","true")}));else{var box=document.getElementById(id),nav,content;if(!box)(box=document.createElement("div")).className="music-tabs",box.id=id,(nav=document.createElement("ul")).className="music-nav-tabs",(content=document.createElement("div")).className="music-tab-content",box.appendChild(nav),box.appendChild(content),group[0].element.parentNode.insertBefore(box,group[0].element);var nav=box.querySelector(".music-nav-tabs"),content=box.querySelector(".music-tab-content");group.forEach((function(item,index){var navItem=document.createElement("li");navItem.className="music-tab"+(0===index?" active":""),navItem.innerHTML='<a href="#">'+(item.title||"播放列表 "+(index+1))+"</a>",navItem.addEventListener("click",(function(e){e.preventDefault(),nav.querySelectorAll(".music-tab").forEach((function(tab){tab.classList.remove("active")})),navItem.classList.add("active"),content.querySelectorAll(".music-tab-pane").forEach((function(pane){pane.classList.remove("active"),pane.style.display="none"})),item.element.classList.add("active"),item.element.style.display="block"})),nav.appendChild(navItem),item.element.className="music-tab-pane"+(0===index?" active":""),content.appendChild(item.element),item.element.setAttribute("data-ready","true")}))}}))};Element.prototype.createChild=function(tag,obj,positon){var child=document.createElement(tag);if(obj)for(var key in obj)"innerHTML"===key?child.innerHTML=obj[key]:key.startsWith("on")?child.addEventListener(key.substring(2),obj[key]):key.startsWith("data-")?child.setAttribute(key,obj[key]):child[key]=obj[key];return"after"===positon?this.parentNode.insertBefore(child,this.nextSibling):"replace"===positon?(this.innerHTML="",this.appendChild(child)):this.appendChild(child),child},Element.prototype.child=function(selector){return this.querySelector(selector)},Element.prototype.find=function(selector){return this.querySelectorAll(selector)},Element.prototype.addClass=function(className){return this.classList.add(className),this},Element.prototype.removeClass=function(className){return this.classList.remove(className),this},Element.prototype.hasClass=function(className){return this.classList.contains(className)},Element.prototype.attr=function(name,value){return void 0!==value?(this.setAttribute(name,value),this):this.getAttribute(name)},Element.prototype.width=function(value){return void 0!==value?(this.style.width=value,this):this.offsetWidth},Element.prototype.height=function(value){return void 0!==value?(this.style.height=value,this):this.offsetHeight},Element.prototype.left=function(){return this.getBoundingClientRect().left},Element.prototype.top=function(){return this.getBoundingClientRect().top},Element.prototype.display=function(value){return this.style.display=value||"block",this};const mediaPlayer=function(t,config){var option={type:"audio",mode:"random",controls:["mode","backward","play-pause","forward","volume"],events:{"play-pause":function(event){source.paused?t.player.play():t.player.pause()},music:function(event){info.el.hasClass("show")?info.hide():(info.el.addClass("show"),playlist.scroll().title())}}},utils={random:function(len){return Math.floor(Math.random()*len)},parse:function(link){var result=[];return[["music.163.com.*song.*id=(\\d+)","netease","song"],["music.163.com.*album.*id=(\\d+)","netease","album"],["music.163.com.*artist.*id=(\\d+)","netease","artist"],["music.163.com.*playlist.*id=(\\d+)","netease","playlist"],["music.163.com.*discover/toplist.*id=(\\d+)","netease","playlist"],["y.qq.com.*song/(\\w+).html","tencent","song"],["y.qq.com.*album/(\\w+).html","tencent","album"],["y.qq.com.*singer/(\\w+).html","tencent","artist"],["y.qq.com.*playsquare/(\\w+).html","tencent","playlist"],["y.qq.com.*playlist/(\\w+).html","tencent","playlist"]].forEach((function(rule){var patt,res=new RegExp(rule[0]).exec(link);null!==res&&(result=[rule[1],rule[2],res[1]])})),result},fetch:function(source){var list=[];return new Promise((function(resolve,reject){source.forEach((function(raw){var meta=utils.parse(raw);if(meta[0]){var skey=JSON.stringify(meta),playlist=store.get(skey);playlist?(list.push.apply(list,JSON.parse(playlist)),resolve(list)):fetch("https://api.i-meto.com/meting/api?server="+meta[0]+"&type="+meta[1]+"&id="+meta[2]+"&r="+Math.random()).then((function(response){return response.json()})).then((function(json){store.set(skey,JSON.stringify(json)),list.push.apply(list,json),resolve(list)})).catch((function(ex){}))}else list.push(raw),resolve(list)}))}))},secondToTime:function(second){var add0=function(num){return isNaN(num)?"00":num<10?"0"+num:""+num},hour=Math.floor(second/3600),min=Math.floor((second-3600*hour)/60),sec=Math.floor(second-3600*hour-60*min);return(hour>0?[hour,min,sec]:[min,sec]).map(add0).join(":")},nameMap:{dragStart:isMobile?"touchstart":"mousedown",dragMove:isMobile?"touchmove":"mousemove",dragEnd:isMobile?"touchend":"mouseup"}},source=null,originTitle=document.title;t.player={_id:utils.random(999999),group:!0,load:function(newList){var d="";for(var el in newList&&newList.length>0?this.options.rawList!==newList&&(this.options.rawList=newList,playlist.clear()):(d="none",this.pause()),buttons.el)buttons.el[el].display(d);return this},fetch:function(){var that=this;return new Promise((function(resolve,reject){if(playlist.data.length>0)resolve();else if(that.options.rawList){var promises=[];that.options.rawList.forEach((function(raw,index){promises.push(new Promise((function(resolve,reject){var group=index,source;raw.list?(that.group=!0,source=raw.list):(group=0,that.group=!1,source=[raw]),utils.fetch(source).then((function(list){playlist.add(group,list),resolve()}))})))})),Promise.all(promises).then((function(){resolve(!0)}))}})).then((function(c){c&&(playlist.create(),controller.create(),that.mode())}))},mode:function(){var total=playlist.data.length;if(total&&playlist.errnum!=total){var step="next"==controller.step?1:-1,getGroupRange=function(){var group=0;playlist.index>=0&&playlist.current()&&(group=playlist.current().group);for(var startIndex=0,endIndex=total-1,i=0;i<total;i++)if(playlist.data[i].group===group){startIndex=i;break}for(var i=startIndex;i<total&&playlist.data[i].group===group;i++)endIndex=i;return{start:startIndex,end:endIndex}},next=function(){var index=playlist.index+step;(index>=total||index<0)&&(index="next"==controller.step?0:total-1),playlist.index=index},nextInGroup=function(){var range=getGroupRange(),index=playlist.index+step;index>range.end?index=range.start:index<range.start&&(index=range.end),index<0&&(index=0),index>=total&&(index=total-1),playlist.index=index},random=function(){var p=utils.random(total);playlist.index!==p?playlist.index=p:next()};switch(this.options.mode){case"random":random();break;case"order":next();break;case"loop":-1==playlist.index?playlist.index=0:nextInGroup();break;case"single":-1==playlist.index&&(playlist.index=0)}this.init()}},switch:function(index){"number"==typeof index&&index!=playlist.index&&playlist.current()&&!playlist.current().error&&(playlist.index=index,this.init())},init:function(){var item=playlist.current();if(item&&!item.error){var playing=!1;source.paused||(playing=!0,this.stop()),source.attr("src",item.url),source.attr("title",item.name+" - "+item.artist),this.volume(store.get("_PlayerVolume")||"0.2"),this.muted(store.get("_PlayerMuted")),progress.create(),"audio"==this.options.type&&preview.create(),1==playing&&this.play()}else this.mode()},play:function(){if(NOWPLAYING&&NOWPLAYING.player.pause(),playlist.current().error)this.mode();else{var that=this;source.play().then((function(){playlist.scroll()})).catch((function(e){}))}},pause:function(){source.pause(),document.title=originTitle},stop:function(){source.pause(),source.currentTime=0,document.title=originTitle},seek:function(time){source.duration&&!isNaN(source.duration)&&isFinite(source.duration)&&(time=Math.max(time,0),time=Math.min(time,source.duration),source.currentTime=time,progress.update(time/source.duration))},muted:function(status){"muted"==status?(source.muted=status,store.set("_PlayerMuted",status),controller.update(0)):(store.del("_PlayerMuted"),source.muted=!1,controller.update(source.volume))},volume:function(percentage){isNaN(percentage)||(controller.update(percentage),store.set("_PlayerVolume",percentage),source.volume=percentage)},mini:function(){info.hide()}};var info={el:null,create:function(){this.el||(this.el=t.createChild("div",{className:"player-info",innerHTML:("audio"==t.player.options.type?'<div class="preview"></div>':"")+'<div class="controller"></div><div class="playlist"></div>'}),preview.el=this.el.child(".preview"),playlist.el=this.el.child(".playlist"),controller.el=this.el.child(".controller"))},hide:function(){var el=this.el;el.addClass("hide"),window.setTimeout((function(){el.removeClass("show hide")}),300)}},playlist={el:null,data:[],index:-1,errnum:0,add:function(group,list){var that=this;list.forEach((function(item,i){item.group=group,item.name=item.name||item.title||"Media name",item.artist=item.artist||item.author||"Anonymous",item.cover=item.cover||item.pic,item.type=item.type||"normal",that.data.push(item)}))},clear:function(){this.data=[],this.el.innerHTML="",-1!==this.index&&(this.index=-1,t.player.fetch())},create:function(){var el=this.el;this.data.map((function(item,index){if(!item.el){var id="list-"+t.player._id+"-"+item.group,tab=document.getElementById(id);return tab||(tab=el.createChild("div",{id:id,className:t.player.group?"tab":"music-tab-pane",innerHTML:"<ol></ol>"}),t.player.group&&tab.attr("data-title",t.player.options.rawList[item.group].title).attr("data-id",t.player._id)),item.el=tab.child("ol").createChild("li",{title:item.name+" - "+item.artist,innerHTML:'<span class="info"><span>'+item.name+"</span><span>"+item.artist+"</span></span>","data-index":index,onclick:function(event){var current=event.currentTarget,globalIndex=parseInt(current.getAttribute("data-index"));playlist.index===globalIndex&&progress.el?source.paused?t.player.play():t.player.seek(source.duration*progress.percent(event,current)):(t.player.switch(globalIndex),t.player.play())}}),item}})),tabFormat()},current:function(){return this.data[this.index]},scroll:function(){var item=this.current(),li=this.el.child("li.current");if(li&&li.removeClass("current"),item&&item.el){item.el.addClass("current");var tab=this.el.child(".music-tab-pane.active");tab&&tab.removeClass("active");var targetTab=item.el.closest(".music-tab-pane");if(targetTab){targetTab.addClass("active");var tabsContainer=targetTab.closest(".music-tabs");if(tabsContainer){var navTabs=tabsContainer.querySelectorAll(".music-tab"),tabPanes=tabsContainer.querySelectorAll(".music-tab-pane"),targetIndex=Array.from(tabPanes).indexOf(targetTab);navTabs.forEach((function(nav,index){index===targetIndex?nav.classList.add("active"):nav.classList.remove("active")})),tabPanes.forEach((function(pane,index){index===targetIndex?(pane.classList.add("active"),pane.style.display="block"):(pane.classList.remove("active"),pane.style.display="none")}))}}var scrollContainer,listContainer=(targetTab||this.el).querySelector("ol");listContainer&&item.el&&setTimeout((function(){for(var scrollableContainer=listContainer;scrollableContainer;){var style=window.getComputedStyle(scrollableContainer);if("auto"===style.overflowY||"scroll"===style.overflowY||"auto"===style.overflow||"scroll"===style.overflow)break;if(scrollableContainer.classList.contains("music-tab-pane")||scrollableContainer.classList.contains("playlist"))break;scrollableContainer=scrollableContainer.parentElement}scrollableContainer||(scrollableContainer=listContainer);var itemRect=item.el.getBoundingClientRect(),containerRect=scrollableContainer.getBoundingClientRect(),itemOffsetTop=item.el.offsetTop,itemHeight=item.el.clientHeight,containerHeight=scrollableContainer.clientHeight,targetScrollTop=scrollableContainer.scrollTop+(itemRect.top-containerRect.top)-containerHeight/2+itemHeight/2,maxScrollTop=scrollableContainer.scrollHeight-containerHeight;scrollableContainer.scrollTo({top:Math.max(0,Math.min(targetScrollTop,maxScrollTop)),behavior:"smooth"})}),100)}return this},title:function(){if(!source.paused){var current=this.current();document.title="Now Playing..."+current.name+" - "+current.artist+" | "+originTitle}},error:function(){var current=this.current();current.el.removeClass("current").addClass("error"),current.error=!0,this.errnum++}},lyrics={el:null,data:null,index:0,create:function(box){if(box){var current=playlist.index,that=this,raw=playlist.current().lrc,callback=function(body){if(current===playlist.index){that.data=that.parse(body);var lrc="";that.data.forEach((function(line,index){lrc+="<p"+(0===index?' class="current"':"")+">"+line[1]+"</p>"})),that.el=box.createChild("div",{className:"inner",innerHTML:lrc},"replace"),that.index=0}};raw&&raw.startsWith("http")?this.fetch(raw,callback):callback(raw||"")}},update:function(currentTime){if(this.data&&this.el&&(this.index>this.data.length-1||currentTime<this.data[this.index][0]||!this.data[this.index+1]||currentTime>=this.data[this.index+1][0]))for(var i=0;i<this.data.length;i++)if(currentTime>=this.data[i][0]&&(!this.data[i+1]||currentTime<this.data[i+1][0])){this.index=i;for(var lrcContainer=this.el.parentElement,lrcLines=this.el.getElementsByTagName("p"),currentElements=this.el.getElementsByClassName("current"),j=0;j<currentElements.length;j++)currentElements[j].classList.remove("current");if(lrcLines[i]&&lrcLines[i].classList.add("current"),lrcContainer&&lrcLines[i]){var containerHeight=lrcContainer.clientHeight,lineHeight=lrcLines[i].clientHeight,lineOffsetTop,scrollY=lrcLines[i].offsetTop-containerHeight/2+lineHeight/2;this.el.style.transition="transform 0.3s ease",this.el.style.transform="translateY("+-scrollY+"px)",this.el.style.webkitTransform="translateY("+-scrollY+"px)"}}},parse:function(lrc_s){if(lrc_s){const lyric=(lrc_s=lrc_s.replace(/([^\]^\n])\[/g,(function(match,p1){return p1+"\n["}))).split("\n");var lrc=[];const lyricLen=lyric.length;for(var i=0;i<lyricLen;i++){const lrcTimes=lyric[i].match(/\[(\d{2}):(\d{2})(\.(\d{2,3}))?\]/g),lrcText=lyric[i].replace(/.*\[(\d{2}):(\d{2})(\.(\d{2,3}))?\]/g,"").replace(/<(\d{2}):(\d{2})(\.(\d{2,3}))?>/g,"").replace(/^\s+|\s+$/g,"");if(lrcTimes){const timeLen=lrcTimes.length;for(var j=0;j<timeLen;j++){const oneTime=/\[(\d{2}):(\d{2})(\.(\d{2,3}))?\]/.exec(lrcTimes[j]),min2sec=60*oneTime[1],sec2sec=parseInt(oneTime[2]),msec2sec=oneTime[4]?parseInt(oneTime[4])/(2===(oneTime[4]+"").length?100:1e3):0,lrcTime=min2sec+sec2sec+msec2sec;lrc.push([lrcTime,lrcText])}}}return(lrc=lrc.filter((function(item){return item[1]}))).sort((function(a,b){return a[0]-b[0]})),lrc}return[]},fetch:function(url,callback){fetch(url).then((function(response){return response.text()})).then((function(body){callback(body)})).catch((function(ex){}))}},preview={el:null,create:function(){var current=playlist.current();this.el.innerHTML='<div class="background"><img src="'+(current.cover||"")+'" alt="Background"/></div><div class="cover"><div class="disc"><img src="'+(current.cover||"")+'" alt="Album Cover"/></div><div class="play-overlay" data-action="play-pause"></div></div><div class="song-info"><div class="title">'+current.name+'</div><div class="artist">'+current.artist+'</div><div class="album"></div></div><div class="lrc"></div><div class="controls"><button class="control-btn lyrics-btn" data-action="lyrics" title="显示/隐藏歌词"></button><button class="control-btn backward" data-action="backward" title="上一首"></button><button class="control-btn play-pause" data-action="play-pause" title="播放/暂停"></button><button class="control-btn forward" data-action="forward" title="下一首"></button><div class="volume-container"><button class="control-btn volume-btn" data-action="volume" title="音量"></button><div class="volume-slider-vertical"><div class="volume-percent">100%</div><div class="slider-track"><div class="slider-progress"></div><div class="slider-thumb"></div></div></div></div></div><div class="bottom-controls"><button class="control-btn mode-btn loop" data-action="mode" title="播放模式"></button><div class="player-bottom-bar"><div class="time-info current">0:00</div><div class="progress-track"><div class="progress-bar"></div></div><div class="time-info">0:00</div></div><button class="control-btn menu" data-action="menu" title="播放列表"></button></div>',this.setupProgressBar(),this.setupVolumeControl();var buttons=this.el.querySelectorAll("[data-action]"),that=this;buttons.forEach((function(btn){var action=btn.getAttribute("data-action");"lyrics"===action?btn.addEventListener("click",(function(e){e.preventDefault(),e.stopPropagation();var lrcContainer=that.el.closest(".player-info").querySelector(".lrc");if(lrcContainer){lrcContainer.classList.toggle("show");var isShow=lrcContainer.classList.contains("show");isShow&&""===lrcContainer.innerHTML.trim()&&lyrics.create(lrcContainer),showtip(isShow?"已开启歌词显示":"已关闭歌词显示")}})):"volume"===action||("play-pause"===action?btn.addEventListener("click",(function(e){e.preventDefault(),e.stopPropagation(),source.paused?t.player.play():t.player.pause()})):"backward"===action||"forward"===action?btn.addEventListener("click",(function(e){e.preventDefault(),e.stopPropagation(),controller.step="backward"===action?"prev":"next",t.player.mode()})):"mode"===action?btn.addEventListener("click",(function(e){e.preventDefault(),e.stopPropagation(),that.handleModeChange(btn)})):"menu"===action&&btn.addEventListener("click",(function(e){e.preventDefault(),e.stopPropagation();var playlistEl=that.el.closest(".player-info").querySelector(".playlist");playlistEl&&(playlistEl.classList.toggle("show"),playlistEl.classList.contains("show")&&playlist.scroll())})))}));var modeBtn=this.el.querySelector(".mode-btn");modeBtn&&(modeBtn.className="control-btn mode-btn "+t.player.options.mode),lyrics.create(this.el.child(".lrc"))},setupVolumeControl:function(){var volumeBtn=document.querySelector(".volume-btn"),volumeSlider=document.querySelector(".volume-slider-vertical"),volumePercent=document.querySelector(".volume-slider-vertical .volume-percent"),sliderTrack=document.querySelector(".volume-slider-vertical .slider-track"),sliderProgress=document.querySelector(".volume-slider-vertical .slider-progress"),sliderThumb=document.querySelector(".volume-slider-vertical .slider-thumb"),that=this;volumeBtn&&sliderTrack?(this.updateVolumeDisplay=function(){var volume=source.muted?0:source.volume,percentage=Math.floor(100*volume);volumePercent&&(volumePercent.textContent=percentage+"%"),sliderProgress&&(sliderProgress.style.height=percentage+"%"),sliderThumb&&(sliderThumb.style.bottom=percentage+"%"),volumeBtn&&(volumeBtn.className="volume-btn"+(source.muted||0===volume?" muted":""))},volumeBtn.addEventListener("click",(function(e){e.stopPropagation(),source.muted?(t.player.muted(),t.player.volume(source.volume||.5)):t.player.muted("muted"),that.updateVolumeDisplay()})),sliderTrack.addEventListener("click",(function(e){e.stopPropagation();var rect=sliderTrack.getBoundingClientRect(),clickY,trackHeight,volume=1-(e.clientY-rect.top)/rect.height;volume=Math.max(0,Math.min(1,volume)),source.muted&&t.player.muted(),t.player.volume(volume),that.updateVolumeDisplay()})),sliderThumb.addEventListener("mousedown",(function(e){e.preventDefault(),e.stopPropagation();var rect=sliderTrack.getBoundingClientRect(),trackHeight=rect.height,isDragging=!1;function onMouseMove(e){isDragging=!0;var clientY,relativeY,volume=1-(e.clientY-rect.top)/trackHeight;volume=Math.max(0,Math.min(1,volume)),source.muted&&t.player.muted(),source.volume=volume,that.updateVolumeDisplay()}function onMouseUp(){isDragging=!1,document.removeEventListener("mousemove",onMouseMove),document.removeEventListener("mouseup",onMouseUp),void 0!==source.volume&&store.set("_PlayerVolume",source.volume)}document.addEventListener("mousemove",onMouseMove),document.addEventListener("mouseup",onMouseUp)})),that.updateVolumeDisplay()):console.error("音量控制元素未找到")},setupProgressBar:function(){var bottomBar=this.el.querySelector(".player-bottom-bar"),track=bottomBar?bottomBar.querySelector(".progress-track"):null,bar=bottomBar?bottomBar.querySelector(".progress-bar"):null,that=this;if(track&&bar){track.addEventListener("click",(function(e){if(e.stopPropagation(),source.duration&&!isNaN(source.duration)&&isFinite(source.duration)){var rect=track.getBoundingClientRect(),clickX,trackWidth,percent=(e.clientX-rect.left)/rect.width;percent=Math.max(0,Math.min(1,percent)),t.player.seek(percent*source.duration)}}));var isDragging=!1;bar.addEventListener("mousedown",(function(e){e.preventDefault(),e.stopPropagation(),isDragging=!0;var rect=track.getBoundingClientRect(),trackWidth=rect.width;function onMouseMove(e){if(isDragging){var clientX,relativeX,percent=(e.clientX-rect.left)/trackWidth;percent=Math.max(0,Math.min(1,percent)),bar.style.width=100*percent+"%"}}function onMouseUp(){if(isDragging=!1,document.removeEventListener("mousemove",onMouseMove),document.removeEventListener("mouseup",onMouseUp),source.duration&&!isNaN(source.duration)&&isFinite(source.duration)){var rect=track.getBoundingClientRect(),trackWidth=rect.width,clientX,relativeX,percent=(event.clientX-rect.left)/trackWidth;percent=Math.max(0,Math.min(1,percent)),t.player.seek(percent*source.duration)}}document.addEventListener("mousemove",onMouseMove),document.addEventListener("mouseup",onMouseUp)}))}else console.error("进度条元素未找到")},setupToggleSwitch:function(bottomBar){var toggleSwitch=bottomBar.querySelector(".toggle-switch"),that=this;toggleSwitch&&toggleSwitch.addEventListener("click",(function(e){e.stopPropagation(),toggleSwitch.classList.toggle("active")}))},updateProgressBar:function(currentTime,duration){var bar=document.querySelector(".player-bottom-bar .progress-bar"),timeInfos=document.querySelectorAll(".player-bottom-bar .time-info");if(bar&&duration){var percent=currentTime/duration;if(bar.style.width=100*percent+"%",timeInfos.length>=2){var currentMin=Math.floor(currentTime/60),currentSec=Math.floor(currentTime%60),totalMin=Math.floor(duration/60),totalSec=Math.floor(duration%60);timeInfos[0].textContent=currentMin+":"+(currentSec<10?"0":"")+currentSec,timeInfos[1].textContent=totalMin+":"+(totalSec<10?"0":"")+totalSec}}},handleModeChange:function(modeBtn){var oldMode=t.player.options.mode,modeTips;switch(t.player.options.mode){case"loop":t.player.options.mode="single";break;case"single":t.player.options.mode="random";break;default:t.player.options.mode="loop"}modeBtn.className=modeBtn.className.replace(/(loop|random|single)/g,t.player.options.mode),store.set("_PlayerMode",t.player.options.mode),showtip("播放模式："+{loop:"列表循环",random:"随机播放",single:"单曲循环",order:"顺序播放"}[t.player.options.mode])}},globalProgress_el=null,globalProgress_bar=null,globalProgress_create=function(){},globalProgress_update=function(currentTime,duration){preview.updateProgressBar&&preview.updateProgressBar(currentTime,duration)},globalProgress_seek=function(e){if(source.duration&&!isNaN(source.duration)&&isFinite(source.duration)){var track=document.querySelector(".progress-track");if(track){var rect=track.getBoundingClientRect(),percent=(e.clientX-rect.left)/rect.width;percent=Math.max(0,Math.min(1,percent)),t.player.seek(percent*source.duration)}}},globalProgress_drag=function(e){if(e.preventDefault(),source.duration&&!isNaN(source.duration)&&isFinite(source.duration)){var track=document.querySelector(".progress-track");if(track){var bar=document.querySelector(".progress-bar"),that=this,thumbMove=function(e){e.preventDefault();var rect=track.getBoundingClientRect(),percent=((e.clientX||e.changedTouches[0].clientX)-rect.left)/rect.width;percent=Math.max(0,Math.min(1,percent)),bar&&(bar.style.width=Math.floor(100*percent)+"%")},thumbUp=function(e){e.preventDefault(),document.removeEventListener(utils.nameMap.dragEnd,thumbUp),document.removeEventListener(utils.nameMap.dragMove,thumbMove);var rect=track.getBoundingClientRect(),percent=((e.clientX||e.changedTouches[0].clientX)-rect.left)/rect.width;percent=Math.max(0,Math.min(1,percent)),t.player.seek(percent*source.duration),source.disableTimeupdate=!1};source.disableTimeupdate=!0,document.addEventListener(utils.nameMap.dragMove,thumbMove),document.addEventListener(utils.nameMap.dragEnd,thumbUp)}}},progress={el:null,bar:null,create:function(){var current=playlist.current().el;current&&(this.el&&(this.el.parentNode.removeClass("current").removeEventListener(utils.nameMap.dragStart,this.drag),this.el.remove()),this.el=current.createChild("div",{className:"progress"}),this.el.attr("data-dtime",utils.secondToTime(0)),this.bar=this.el.createChild("div",{className:"bar"}),current.addClass("current"),current.addEventListener(utils.nameMap.dragStart,this.drag),playlist.scroll())},update:function(percent){this.bar.width(Math.floor(100*percent)+"%"),this.el.attr("data-ptime",utils.secondToTime(percent*source.duration))},seeking:function(type){type?this.el.addClass("seeking"):this.el.removeClass("seeking")},percent:function(e,el){var percentage=((e.clientX||e.changedTouches[0].clientX)-el.left())/el.width();return percentage=Math.max(percentage,0),Math.min(percentage,1)},drag:function(e){e.preventDefault();var current=playlist.current().el,thumbMove=function(e){e.preventDefault();var percentage=progress.percent(e,current);progress.update(percentage),lyrics.update(percentage*source.duration)},thumbUp=function(e){e.preventDefault(),current.removeEventListener(utils.nameMap.dragEnd,thumbUp),current.removeEventListener(utils.nameMap.dragMove,thumbMove);var percentage=progress.percent(e,current);progress.update(percentage),t.player.seek(percentage*source.duration),source.disableTimeupdate=!1,progress.seeking(!1)};source.disableTimeupdate=!0,progress.seeking(!0),current.addEventListener(utils.nameMap.dragMove,thumbMove),current.addEventListener(utils.nameMap.dragEnd,thumbUp)}},controller={el:null,btns:{},step:"next",create:function(){if(t.player.options.controls){globalProgress_create();var that=this;t.player.options.controls.forEach((function(item){if(!that.btns[item]){var opt={className:item+" btn",onclick:function(event){that.events[item]?that.events[item](event):t.player.options.events[item](event)}};that.btns[item]=that.el.createChild("div",opt)}}))}},events:{mode:function(e){switch(t.player.options.mode){case"loop":t.player.options.mode="single";break;case"single":t.player.options.mode="random";break;default:t.player.options.mode="loop"}controller.btns.mode.className="mode "+t.player.options.mode+" btn",store.set("_PlayerMode",t.player.options.mode)},backward:function(e){controller.step="prev",t.player.mode()},forward:function(e){controller.step="next",t.player.mode()}},update:function(percent){controller.btns.volume&&(controller.btns.volume.className="volume "+(!source.muted&&percent>0?"on":"off")+" btn",controller.btns.volume.bar&&controller.btns.volume.bar.width(Math.floor(100*percent)+"%"),preview.updateVolumeDisplay&&preview.updateVolumeDisplay())},percent:function(e,el){var rect=el.getBoundingClientRect(),clientX,percentage=((e.clientX||e.changedTouches&&e.changedTouches[0].clientX)-rect.left)/rect.width;return Math.max(0,Math.min(1,percentage))}},events={onerror:function(){playlist.error(),t.player.mode()},ondurationchange:function(){1!==source.duration&&progress.el.attr("data-dtime",utils.secondToTime(source.duration))},onloadedmetadata:function(){t.player.seek(0),progress.el.attr("data-dtime",utils.secondToTime(source.duration)),globalProgress_update(0,source.duration)},onplay:function(){t.parentNode.addClass("playing"),showtip(this.attr("title")),NOWPLAYING=t},onpause:function(){t.parentNode.removeClass("playing"),NOWPLAYING=null},ontimeupdate:function(){this.disableTimeupdate||(progress.update(this.currentTime/this.duration),globalProgress_update(this.currentTime,this.duration),lyrics.update(this.currentTime))},onended:function(argument){controller.step="next",t.player.mode(),t.player.play()}},buttons={el:{},create:function(){if(t.player.options.btns){var that=this;t.player.options.btns.forEach((function(item){that.el[item]||(that.el[item]=t.createChild("div",{className:item+" btn",onclick:function(event){t.player.fetch().then((function(){t.player.options.events[item](event)}))}}))}))}}},init;return function(config){t.player.created||(t.player.options=Object.assign(option,config),t.player.options.mode=store.get("_PlayerMode")||t.player.options.mode,buttons.create(),source=t.createChild(t.player.options.type,events),info.create(),t.parentNode.addClass(t.player.options.type),t.player.created=!0)}(config),t};



//InsightSearch.js
!function($,CONFIG){var $main=$(".ins-search"),$input=$main.find(".ins-search-input"),$wrapper=$main.find(".ins-section-wrapper"),$container=$main.find(".ins-section-container");function section(title){return $("<section>").addClass("ins-section").append($("<header>").addClass("ins-section-header").text(title))}function searchItem(icon,title,slug,preview,url){return $("<div>").addClass("ins-selectable").addClass("ins-search-item").append($("<header>").append($("<i>").addClass("fa").addClass("fa-"+icon)).append(null!=title&&""!=title?title:CONFIG.TRANSLATION.UNTITLED).append(slug?$("<span>").addClass("ins-slug").text(slug):null)).append(preview?$("<p>").addClass("ins-search-preview").text(preview):null).attr("data-url",url)}function sectionFactory(type,array){var sectionTitle,$searchItems;if(0===array.length)return null;switch(sectionTitle=CONFIG.TRANSLATION[type],type){case"POSTS":case"PAGES":$searchItems=array.map((function(item){return searchItem("file",item.title,null,item.text.slice(0,150),CONFIG.ROOT_URL+item.path)}));break;case"CATEGORIES":case"TAGS":$searchItems=array.map((function(item){return searchItem("CATEGORIES"===type?"folder":"tag",item.name,item.slug,null,item.permalink)}));break;default:return null}return section(sectionTitle).append($searchItems)}function extractToSet(json,key){var values={},entries;json.pages.concat(json.posts).forEach((function(entry){entry[key]&&entry[key].forEach((function(value){values[value.name]=value}))}));var result=[];for(var key in values)result.push(values[key]);return result}function parseKeywords(keywords){return keywords.split(" ").filter((function(keyword){return!!keyword})).map((function(keyword){return keyword.toUpperCase()}))}function filter(keywords,obj,fields){var result=!1,keywordArray=parseKeywords(keywords),containKeywords;return keywordArray.filter((function(keyword){var containFields;return fields.filter((function(field){return!!obj.hasOwnProperty(field)&&(obj[field].toUpperCase().indexOf(keyword)>-1||void 0)})).length>0})).length===keywordArray.length}function filterFactory(keywords){return{POST:function(obj){return filter(keywords,obj,["title","text"])},PAGE:function(obj){return filter(keywords,obj,["title","text"])},CATEGORY:function(obj){return filter(keywords,obj,["name","slug"])},TAG:function(obj){return filter(keywords,obj,["name","slug"])}}}function weight(keywords,obj,fields,weights){var value=0;return parseKeywords(keywords).forEach((function(keyword){var pattern=new RegExp(keyword,"img");fields.forEach((function(field,index){if(obj.hasOwnProperty(field)){var matches=obj[field].match(pattern);value+=matches?matches.length*weights[index]:0}}))})),value}function weightFactory(keywords){return{POST:function(obj){return weight(keywords,obj,["title","text"],[3,1])},PAGE:function(obj){return weight(keywords,obj,["title","text"],[3,1])},CATEGORY:function(obj){return weight(keywords,obj,["name","slug"],[1,1])},TAG:function(obj){return weight(keywords,obj,["name","slug"],[1,1])}}}function search(json,keywords){var WEIGHTS=weightFactory(keywords),FILTERS=filterFactory(keywords),posts=json.posts,tags=extractToSet(json,"tags"),categories=extractToSet(json,"categories");return{posts:posts.filter(FILTERS.POST).sort((function(a,b){return WEIGHTS.POST(b)-WEIGHTS.POST(a)})).slice(0,20),categories:categories.filter(FILTERS.CATEGORY).sort((function(a,b){return WEIGHTS.CATEGORY(b)-WEIGHTS.CATEGORY(a)})).slice(0,5),tags:tags.filter(FILTERS.TAG).sort((function(a,b){return WEIGHTS.TAG(b)-WEIGHTS.TAG(a)})).slice(0,5)}}function searchResultToDOM(searchResult){for(var key in $container.empty(),searchResult)$container.append(sectionFactory(key.toUpperCase(),searchResult[key]))}function scrollTo($item){if(0!==$item.length){var wrapperHeight=$wrapper[0].clientHeight,itemTop=$item.position().top-$wrapper.scrollTop(),itemBottom=$item[0].clientHeight+$item.position().top;itemBottom>wrapperHeight+$wrapper.scrollTop()&&$wrapper.scrollTop(itemBottom-$wrapper[0].clientHeight),itemTop<0&&$wrapper.scrollTop($item.position().top)}}function selectItemByDiff(value){var $items=$.makeArray($container.find(".ins-selectable")),prevPosition=-1;$items.forEach((function(item,index){$(item).hasClass("active")&&(prevPosition=index)}));var nextPosition=($items.length+prevPosition+value)%$items.length;$($items[prevPosition]).removeClass("active"),$($items[nextPosition]).addClass("active"),scrollTo($($items[nextPosition]))}function gotoLink($item){$item&&$item.length&&(location.href=$item.attr("data-url"))}$main.parent().remove(".ins-search"),$("body").append($main),$.getJSON(CONFIG.CONTENT_URL,(function(json){"#ins-search"===location.hash.trim()&&$main.addClass("show"),$input.on("input",(function(){var keywords=$(this).val();searchResultToDOM(search(json,keywords))})),$input.trigger("input")})),$(document).on("click focus",".search-field",(function(){$main.addClass("show"),$main.find(".ins-search-input").focus()})).on("click focus",".search-form-submit",(function(){$main.addClass("show"),$main.find(".ins-search-input").focus()})).on("click",".ins-search-item",(function(){gotoLink($(this))})).on("click",".ins-close",(function(){$main.removeClass("show")})).on("keydown",(function(e){if($main.hasClass("show"))switch(e.keyCode){case 27:$main.removeClass("show");break;case 38:selectItemByDiff(-1);break;case 40:selectItemByDiff(1);break;case 13:gotoLink($container.find(".ins-selectable.active").eq(0))}}))}(jQuery,window.INSIGHT_CONFIG);
